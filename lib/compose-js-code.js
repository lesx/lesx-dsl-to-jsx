...
exports.default = ({
    ...
    // 获取js中所有未声明的变量
    let undeclaredVars = (0, (_lesxUndeclaredVars || _load_lesxUndeclaredVars()).default)(jsCode);
    ...
    if (Array.isArray(undeclaredVars) && undeclaredVars.length) {
        ...
        if (Array.isArray(undeclaredVars) && undeclaredVars.lengt) {
            (0, (_lesxCodeInject || _load_lesxCodeInject()).insertCodeToMethod)(jsCode, [{
                name: 'render',
                code: `
                    const ${undeclaredVars.join(', ')} = this;
                `,
                pos: 'prev'
            }]);
        }
